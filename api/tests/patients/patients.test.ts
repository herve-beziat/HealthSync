  ...
  describe("Patients CRUD (admin)", () => {
    ...
    beforeAll(async () => {
      ...
      for (const email of [patientCredentials.email, deletablePatientEmail, patientWithAppointmentEmail, otherPatientEmail]) {
    ...
    describe("GET /patients/:id", () => {
      ...
      it("should return 404 when the id belongs to a non-patient account", async () => {
        const res = await app.request(`/patients/${(await prisma.users.findUniqueOrThrow({ where: { email: adminUser.email } })).id}`, {
          headers: { Authorization: `Bearer ${adminToken}` },
        });
        expect(res.status).toBe(404);
      });
